...
const Coin = ({ coin, toggleDark }) => {
  return (
    <div
      className={`max-w-7xl w-11/12 text-center mx-auto py-2 px-2 grid grid-cols-1 md:grid-cols-5 grid-rows-4 md:grid-rows-1 flex items-center 
      gap-2 rounded-xl my-3 drop-shadow-lg ${toggleDark ? "text-white bg-neutral-900" : "bg-slate-200"}`}
    >
      <div>
        <h2 className="inline md:hidden">Market Cap Rank:</h2>
        <div className={toggleDark ? "inline px-3 ml-1 rounded-full bg-gray-100 text-black p-1" : "inline px-3 ml-1 rounded-full bg-cyan-700 text-white p-1"}>{coin.market_cap_rank}</div>
      </div>
